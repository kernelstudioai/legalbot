...
- `--npm-path` defaults to `LEGALBOT_NPM_PATH` when exported. Under `sudo`, the provisioner first tries `su - "$SUDO_USER" -c 'command -v npm'` so `nvm` installs stay discoverable, then falls back to `command -v npm`.
...
- If Node 22 is managed by `nvm`, the provisioner now tries to resolve npm from the original sudo user before falling back to root-path discovery. Pass `--npm-path /absolute/path/to/npm` if you want to pin it explicitly.
