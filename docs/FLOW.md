...
5. `decideNextAction` produces a runtime decision. For `client` routing it now delegates to `src/runtime/client/clientRuntime.ts`, which can read and update consent state through an injected consent-only persistence adapter and can advance a separate injected intake-state adapter.
...
- M14 adds a consent-gated intake skeleton under `src/runtime/client/intake.ts`.
...
- Intake stores only explicitly accepted structured fields after consent is granted. It does not persist transcripts, raw message bodies, attachments, or create cases.
- If no dedicated intake persistence is injected, the intake skeleton remains in process-local injected memory only.
...
- Intake state remains separate from both consent-state persistence and technical persistence.
